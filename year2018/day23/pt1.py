...
with open("input.txt") as f:
    ...
    strongest_bot = max(bots, key=lambda bot: bot.strength)

    in_range_count = 0
    for bot in bots:
        distance = abs(bot.x - strongest_bot.x) + abs(
            bot.y - strongest_bot.y) + abs(bot.z - strongest_bot.z)
        if distance <= strongest_bot.strength:
            in_range_count += 1
